...
/**
 * Convert a filesystem path to a codebase-memory-mcp project name.
 *
 * The CLI derives project names by stripping the leading `/` and
 * replacing all remaining `/` with `-`.
 *
 * Examples:
 *   /root/projects/pi-pedstack  → root-projects-pi-pedstack
 *   /root/.aru/wt/mimo-v2/feat-dashboard1 → root-.aru-wt-mimo-v2-feat-dashboard1
 */
export function pathToProjectName(path: string): string {
