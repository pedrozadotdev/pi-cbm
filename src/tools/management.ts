...
export function registerManagementTools(
  ...
  pi.registerTool({
    ...
    promptSnippet:
      "List all indexed projects in the codebase knowledge graph",
  ...
  pi.registerTool({
    ...
    promptSnippet:
      "Remove a project's graph index from codebase-memory-mcp",
    parameters: Type.Object({
      project: Type.String({
        description: "Project name or path to remove from the index",
      }),
    }),
  ...
  pi.registerTool({
    ...
    async execute(_id, params, signal) {
      const result = await runCbm(pi, cbmBin, "manage_adr", params, signal);
      return {
        content: [{ type: "text", text: formatResult(result) }],
        details: { result },
      };
