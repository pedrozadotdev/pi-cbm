...
export function registerSearchTools(
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      name_pattern: Type.Optional(
        Type.String({
          description:
            "Regex pattern to match node names (e.g. '.*Handler.*')",
        })
      ),
      label: Type.Optional(
        Type.String({
    ...
    async execute(_id, params, signal) {
      const result = await runCbm(pi, cbmBin, "search_graph", params, signal);
      return {
  ...
  pi.registerTool({
    ...
    async execute(_id, params, signal) {
      const result = await runCbm(pi, cbmBin, "search_code", params, signal);
      return {
        content: [{ type: "text", text: formatResult(result) }],
        details: { result },
      };
